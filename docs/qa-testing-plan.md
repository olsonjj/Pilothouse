...
- [ ] 3.2 Quarter dropdowns (Rocks, Issues) list current + next year's quarters.

## 4. Accountability Chart (tickets 04, 08)
...
- [ ] 10.6 Origin badges: create an issue from a rock/scorecard/todo (via L10
      push, section 13) → badge "from rock"/"from scorecard"/"from to-do".
...
## 11. Rocks (tickets 17–19)

- [ ] 11.1 Add a company rock (admin) and a personal rock (member, "Me" only).
- [ ] 11.2 8th rock in a quarter → yellow warning banner (never blocks).
- [ ] 11.3 Target without direction (or vice versa) → rejected.
- [ ] 11.4 Weekly status: ✓ / ✗ / 📊 per rock per week; re-entering the same
      week overwrites; comment saved.
- [ ] 11.5 📊 measuring requires the rock to have a target; captures the
      actual number.
...
- [ ] 11.7 Past quarter: everything read-only; admin sees ✓/✗ scoring buttons
      and the completion-percentage bars (EOS ~80% reference); unscored rocks
      count as incomplete.
- [ ] 11.8 Carry over an unfinished rock to the next quarter → new rock with a
      "carried" badge; original untouched.

## 12. L10 Meeting (tickets 21–25)

- [ ] 12.1 Start meeting → 7 segments in agenda order (Segue 5 / Scorecard 5 /
      Rocks 5 / Headlines 5 / To-Dos 5 / IDS 60 / Conclude 5), first active
      with countdown.
- [ ] 12.2 Pre-loads: Scorecard segment shows last week's column with pass/fail;
      Rocks shows current statuses; To-Dos shows last week's bucket.
...
- [ ] 12.8 Push: "Make issue" on a red scorecard cell, an off-track rock, and a
      missed to-do → each lands in the IDS queue with the right origin badge.
...
| Action | admin | member |
|---|---|---|
| Edit V/TO, seats, GWC, People Analyzer, company rocks, any metric entry | ✅ | ❌ |
| View everything except other people's Analyzer scores | ✅ | ✅ |
| Create own rocks, enter own metrics, add/resolve issues, push/solve in L10 | ✅ | ✅ |
| Conclude meeting, carry issues, score rocks at quarter end | ✅ | ❌ |
...
- Measuring rocks can't be pushed to IDS in v1; `from_meeting` origin unused.
