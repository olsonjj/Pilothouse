# 15: Scorecard: trends & rollups

**What to build:** Glanceable history: per-metric 8–12 week trend as simple CSS bars colored pass/fail, and a trailing-quarter "weeks on-track %" rollup per metric and per owner. No chart library.

**Blocked by:** 14: Scorecard: weekly grid.

**Status:** done (reviewer sign-off; boundary pins verified flipping under off-by-one)

- [x] Trend view shows the last 8–12 weeks per metric as colored bars
- [x] Trailing-quarter on-track % per metric and per owner
- [x] Pure percentage/date math unit-tested directly where boundary-heavy
- [x] Renders acceptably on narrow screens (responsive)
